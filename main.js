...
class GameObj {
    // constructor
    constructor(propX, propY, propWidth, propHeight, imgArr) {
        this.x = propX;
        this.y = propY;
...
let man = new GameObj(manInitLocX, manInitLocY, manInitSizeW, manInitSizeH, manImgArr);
let dino = new GameObj(dinoInitLocX, dinoInitLocY, dinoInitSizeW, dinoInitSizeH, dinoImgArr);
let fireballArr = [];
...
function runAnimation() {
    ...
    fireballArr.forEach((fireball, index, arr) => {
        if (fireball.x < 0)
            arr.splice(index, 1); // fireball 사라짐
    ...
    if (jumpingStatus) {
        ...
        if (man.y < maxJumpHeight)
            man.y += 3;
        else if (man.y >= maxJumpHeight)
            jumping_timer = 0;
    }
    if (jumping_timer > 25)
        jumpingStatus = false;
